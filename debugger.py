...
class Debugger(Solution):
    ...
    def check(self, node, nums, loops=[0]):
        nodes = self.bft(node.root)
        try:
            count = 0
            for node in nodes:
                count += 1
                if node.left:
                    if node.left.val > node.val:
                        raise BSTError('Value on the wrong side')

                if node.right:
                    if node.right.val < node.val:
                        raise BSTError('Value on the wrong side')

                if node.root.size + loops[0] != len(nums):
                    raise BSTError('Wrong root size')

            if count - loops[0] != node.root.size:
                raise BSTError('Missing nodes expected:{} got:{}'.format(node.root.size, count))
        except BSTError:
            self.display(node.root)
            print 'LOOPS', loops[0]
            raise
    ...
    def display(self, node):
        ...
        while row:
            ...
            for node in row:
                if node in visited:
                    raise BSTError('Cycle found in tree')
                    continue
